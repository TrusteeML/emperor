...
class DQNPolicy:
    ...
    def predict_serialized(self, serialized_states):
        y = []
        for serialized_state in serialized_states:
            # reconstructing deserialized state
            state = [np.zeros((self.parameters["S_INFO"], self.parameters["S_LEN"]))]
            # dequeue history record
            state = np.roll(state, -1, axis=1)[0]

            state[0, -1] = serialized_state[0]
            state[1, -1] = serialized_state[1]
            for i in range(self.parameters["S_LEN"]):
                state[2, i] = serialized_state[2 + i]
            for i in range(self.parameters["S_LEN"]):
                state[3, i] = serialized_state[self.parameters["S_LEN"] + 2 + i]
            for i in range(self.parameters["A_DIM"]):
                state[4, i] = serialized_state[
                    self.parameters["S_LEN"] + self.parameters["S_LEN"] + 2 + i
                ]
            state[5, -1] = serialized_state[
                self.parameters["A_DIM"]
                + self.parameters["S_LEN"]
                + self.parameters["S_LEN"]
                + 2
            ]
            ...
            action_prob = self.actor.predict(
                np.reshape(
                    state, (1, self.parameters["S_INFO"], self.parameters["S_LEN"])
                )
            )
            action_cumsum = np.cumsum(action_prob)
            bit_rate = (
                action_cumsum
                > np.random.randint(1, self.parameters["RAND_RANGE"])
                / float(self.parameters["RAND_RANGE"])
            ).argmax()
            y.append(bit_rate)
        return y
