...
for file in $FILES; do
    ...
    # tshark -r ${file} -Tfields -E occurrence=f -E separator=, -e frame.len -e eth.type -e ip.proto -e ip.flags -e ipv6.nxt -e ipv6.opt -e tcp.flags -e eth.src > ${ofile}.csv
    cp ${ofile}.csv  else.csv
    while IFS= read -r line
    do
      MAC="$(echo $line |cut -d/ -f1)"
      grep -i $MAC ${ofile}.csv |gsed  "s/$line/gI" >>tmp
      grep -v -i $MAC else.csv >else1.csv
      mv else1.csv else.csv
    done < "$1"
    mv tmp ${TARGET}/${ofile}-labeled.csv
    rm ${ofile}.csv
    cat else.csv | gsed 's/.\{17\}$/4/' >> ${TARGET}/${ofile}-labeled.csv
    cat ${TARGET}/${ofile}-labeled.csv |gsed 's/,,/,-1,/gI' | gsed 's/,,/,-1,/gI' > ${TARGET}/${ofile}-labeled.csv1
