...
class Trepan:
    ...
    def plot(root, path=""):
        ...
        dot = Trepan._toDot(root, None, None, dot)
    ...
    def _toDot(node, parent, branchLabel, dot):
        if node.leaf:
            dot.node(str(node.dominant), str(node.dominant))
            if parent:
                dot.edge(str(parent.splitrule), str(node.dominant), branchLabel)
        else:
            dot.node(str(node.splitrule), str(node.splitrule))
            if parent:
                dot.edge(str(parent.splitrule), str(node.splitrule), branchLabel)

            Trepan._toDot(node.left_child, node, "false", dot)
            Trepan._toDot(node.right_child, node, "true", dot)
    ...
    def build_tree(MIN_EXAMPLES_PER_NODE, MAX_NODES, trainX, oracle):
        ...
        labels = oracle.predict(trainX)
        # for i in range(0, trainX.shape[0]):
        #     labels[i] = oracle.get_oracle_label(trainX.iloc[i, :])
        ...
        sortedQueue.put(
            (root.priority, 0, root, all_examples, Constraints(num_dimensions))
        )

        print("SORTED QUEUE INITIALIZED", sortedQueue)
        num_nodes = 1
        while not sortedQueue.empty():
            ...
            if num_examples < MIN_EXAMPLES_PER_NODE:
                ...
                (
                    trainX_oracle,
                    labels_oracle,
                ) = oracle.generate_constrained_examples_with_labels(
                    constraints, num_required
                )
            ...
            sortedQueue.put(
                (priority, num_nodes, left_child, examples_l, constraints_left)
            )
            num_nodes += 1
            ...
            sortedQueue.put(
                (priority, num_nodes, right_child, examples_r, constraints_right)
            )
            num_nodes += 1
...
class SplitFinder:
    ...
    def mutual_information(X, y):
		...
		"""
        for idx in np.flip(sorted_indices):
            label = y[idx]

            # case : identical value
            if prev_idx and X.iloc[prev_idx] == X.iloc[idx]:
                gains[idx] = gains[prev_idx]
            else:
                frac_right = float(shifted) / num_examples
                frac_left = 1 - frac_right
                gains[idx] = entropy_parent
                gains[idx] -= frac_left * SplitFinder.entropy(
                    left_frequencies, num_examples
                )
                gains[idx] -= frac_right * SplitFinder.entropy(
                    right_frequencies, num_examples
                )
            shifted += 1
    ...
    def find_best_single_feature_split(examples):
        ...
        for i in range(0, num_dimensions):
            gains[:, i] = SplitFinder.mutual_information(
                np.reshape(X.iloc[:, i], num_examples), np.reshape(labels, num_examples)
            )
        ...
        split_value = X.iloc[split_point]

        # avoid making a trivial split
        if (X.iloc[:, feature_to_split] <= split_value).all() or (
            X.iloc[:, feature_to_split] >= split_value
        ).all():
            return None
...
def partition(examples, srule):
    ...
    for idx in range(0, num_examples):
        if srule.satisfied(X.iloc[idx, :]):
            left_partition.append(idx)
        else:
            right_partition.append(idx)

    examples_l = (X.iloc[left_partition, :], y[left_partition])
    examples_r = (X.iloc[right_partition, :], y[right_partition])
...
class Oracle:
    ...
    def generate_feature_distributions(self, trainX):
        ...
        for i in range(0, self.dimension):
            feature_values = np.array(trainX.iloc[:, i]).reshape(trainX.shape[0])
            if len(np.unique(feature_values)) <= 2:
                self.categorical.add(i)
    ...
    def get_oracle_label(self, example):
        ...
        """
        return self.network.predict([example])
    ...
    def predict(self, examples):
        """
        Returns the label predicated by the oracle network for examples
        """
        return self.network.predict(examples)
    ...
    def generate_constrained_example(self, constraints):
        ...
        for i in range(0, self.dimension):
            ...
            if i in self.categorical:
                example[i] = np.random.choice([0, 1])

            if i >= len(self.feature_distributions):
                done = True

            # generate the ith feature by rejection sampling
            while not done:
                print("example", i, len(example), len(self.feature_distributions))
                print("resample", self.feature_distributions[i].resample(1))
