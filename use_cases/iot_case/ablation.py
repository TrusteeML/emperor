...
def plot_results_no_contraints():
    ...
    with open(f"{output_dir}/ablation_no_contraints.csv", "r") as f:
        reader = csv.reader(f)
        next(reader)
        for line in reader:
            (
    ...
    plots_dir = f"{OUTPUT_PATH}/ablation/plots_no_contraints"
...
def plot_results_max_leaves():
    ...
    output_dir = f"{OUTPUT_PATH}/ablation"
    with open(f"{output_dir}/ablation_max_leaves.csv", "r") as f:
        reader = csv.reader(f)
        next(reader)
        for line in reader:
            (
    ...
    plots_dir = f"{OUTPUT_PATH}/ablation/plots_max_leaves"
    if not os.path.exists(plots_dir):
        os.makedirs(plots_dir)
...
def compare_methods_no_contraints(
    ...
    """Compares different ablation approaches to ma-trustee with no constraints"""
    num_iter = 50
    train_size = 0.7
    with open(f"{output_dir}/ablation_no_contraints.csv", "w", encoding="utf-8") as f:
...
def compare_methods_max_leaves(
    ...
    samples_size = 0.3
    num_quantiles = 10
    with open(f"{output_dir}/ablation_max_leaves.csv", "w", encoding="utf-8") as csv_file:
        # create the csv writer
...
def main():
    ...
    logger = log.Logger("{}/ablation.log".format(OUTPUT_PATH))
