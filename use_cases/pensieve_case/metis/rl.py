...
def get_reward(bit_rate, rebuf, last_bit_rate, parameters, reward_type):
    if reward_type == "LIN":
        ...
        reward = (
            log_bit_rate
            - parameters["REBUF_PENALTY"] * rebuf
            - parameters["SMOOTH_PENALTY"] * np.abs(log_bit_rate - log_last_bit_rate)
        )
    elif reward_type == "HD":
        reward = (
...
def get_rollout(env, policy, parameters, is_student=True):
    ...
    while True:
        ...
        para_copy = [
            int(env.trace_idx),
            int(env.mahimahi_ptr),
            int(env.video_chunk_counter),
            env.buffer_size,
        ]
...
def _sample(obss, acts, qs, copies, max_pts, is_reweight):
    ...
    ps = ps / np.sum(ps) if np.sum(ps) > 0 else 0
...
def train_dagger(
    ...
    for i in range(max_iters):
        ...
        cur_serialized_obss, cur_acts, cur_qs, cur_copies = _sample(
            np.array(serialized_obss),
            np.array(acts),
            np.array(qs),
            np.array(copies),
            max_samples,
