...
def main():
    ...
    plt.ylabel("Total Reward", fontname=FONT_NAME, fontweight=FONT_WEIGHT)
    plt.xlabel("Trace Index", fontname=FONT_NAME, fontweight=FONT_WEIGHT)
    # plt.show()
    # plt.tight_layout()
    ...
    plt.ylabel("CDF", fontname=FONT_NAME, fontweight=FONT_WEIGHT)
    plt.xlabel("Total Reward", fontname=FONT_NAME, fontweight=FONT_WEIGHT)
    # plt.show()
    # plt.tight_layout()
    ...
    for l in time_all[SCHEMES[0]]:
        ...
        if schemes_check:
            ...
            plt.title(l, fontname=FONT_NAME, fontweight=FONT_WEIGHT, fontsize=18)
            plt.ylabel("Bit-rate \nSelection (kbps)", fontname=FONT_NAME, fontweight=FONT_WEIGHT, fontsize=12)
            ...
            for i, j in enumerate(ax.lines):
                j.set_color(colors[i])
            plt.ylabel("Bandwidth (mbps)", fontname=FONT_NAME, fontweight=FONT_WEIGHT, fontsize=12)
            plt.xlabel("Time (ms)", fontname=FONT_NAME, fontweight=FONT_WEIGHT, fontsize=12)

            plt.tick_params(axis="both", which="major", labelsize=10)

            SCHEMES_REW = []
            for scheme in SCHEMES:
                scheme_name = "trustee" if scheme == "emperor" else scheme
                if scheme == SIM_DP:
                    SCHEMES_REW.append("{}: {:.4f}".format(scheme_name.capitalize(), raw_reward_all[scheme][l]))
                else:
                    SCHEMES_REW.append(
                        "{}: {:.4f}".format(scheme_name.capitalize(), np.sum(raw_reward_all[scheme][l][1:VIDEO_LEN]))
                    )
