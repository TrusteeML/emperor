...
vector<string> IP_HEADERS = {"protocol", "src_ip", "dst_ip", "src_prt", "dst_prt", "ttl"};
...
int main(int argc, char* argv[])
{
    ...
    for (const auto & entry : fs::recursive_directory_iterator(PCAPS_FOLDER)) {
        if (!is_directory(entry.path())) {
            ...
            for (int packetCount = 0; packetCount < 5; packetCount++) {
                ...
                if (parsed_packet.isPacketOfType(pcpp::IPv4)) {
                    ...
                    if (tcp_layer != nullptr) {
                        ...
                        packet_info = {
                            ...
                            src_ip, 
                            dst_ip, 
                            to_string(tcp_layer->getSrcPort()),
                            to_string(tcp_layer->getDstPort()),
                            to_string(ttl), 
                            // headers
                    ...
                    if (udp_layer != nullptr) {
                        ...
                        packet_info = {
                            ...
                            src_ip, 
                            dst_ip,
                            to_string(udp_layer->getSrcPort()),
                            to_string(udp_layer->getDstPort()),
                            to_string(ttl), 
                            // headers
                            "-1",
                            "-1",
